...
def axis_max_s(ts: tsgm.types.Tensor, axis: int) -> tsgm.types.Tensor:
    _validate_axis(axis)
...
def axis_min_s(ts: tsgm.types.Tensor, axis: int) -> tsgm.types.Tensor:
    _validate_axis(axis)
...
def axis_mean_s(ts: tsgm.types.Tensor, axis: int) -> tsgm.types.Tensor:
    _validate_axis(axis)

    return np.mean(ts, axis=axis).flatten()


def axis_mode_s(ts: tsgm.types.Tensor, axis: int) -> tsgm.types.Tensor:
    _validate_axis(axis)

    return scipy.stats.mode(ts, axis=axis)[0].flatten()


def axis_percentile_s(ts: tsgm.types.Tensor, axis: int, percentile: float) -> tsgm.types.Tensor:
    _validate_axis(axis)
